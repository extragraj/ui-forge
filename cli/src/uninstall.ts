...
import { fromPosix, removeFile, removeDir, pruneEmptyDirs } from './fs-utils.js';
...
export async function runUninstall(cwd: string, flags: ParsedFlags): Promise<void> {
  ...
  if (existsSync(shim) && !flags.dryRun) removeFile(shim);
