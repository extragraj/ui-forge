...
import { resolveAssets, FEATURE_DISPLAY, STANDARDS_BY_THEME, type FeatureId } from './assets.js';
...
import { loadLockfile, saveLockfile, type Lockfile } from './lockfile.js';
...
import {
  ...
  pruneOrphanedCommands,
  prunePlatforms,
  pruneOrphanedSkillDirs,
} from './feature-prune.js';
...
export async function runInit(cwd: string, flags: ParsedFlags): Promise<InstallResult> {
  ...
  const addToGroup = (group: string, rel: string) => {
  ...
  if (!flags.dryRun && platformPruneResult.removed.length > 0) {
    ...
    console.log(
      `Platform prune: removed ${platformPruneResult.removed.length} file(s) from deselected platform(s): ${deselectedIds.join(', ')}.`
    );
  }
        ...
        const dests = copyAsset(sourceRoot, skillDir, rel, {
          transform: (assetRel, contents) => {
            const result = maybeTransformAsset(
              { theme: selections.theme, paired: selections.paired, version },
              assetRel,
              contents
            );
  ...
  for (const rel of cmds.written) addToGroup('commands', rel);
  ...
  const themeChanged =
    Boolean(prior) && (prior!.theme !== selections.theme || prior!.paired !== selections.paired);
  ...
  for (const rel of bootstrap.copied) addToGroup('design-standards', rel);
  ...
  if (!flags.dryRun) {
    ...
    //     fallbacks (or AI synthesis) appropriate to the current theme.
    {
      ...
      const arch = readJsonSafe<Record<string, unknown>>(archPath);
      if (arch) {
        ...
        const tailwind = arch.tailwind && typeof arch.tailwind === 'object'
          ? (arch.tailwind as Record<string, unknown>)
          : null;
        const designStandards =
          arch.designStandards && typeof arch.designStandards === 'object'
        ...
        const staleStandardKeys = designStandards
          ? Object.keys(designStandards).filter(
              (k) => themeStandardKeys.has(k) && k !== keepStandardKey
            )
          : [];

        const hasStaleTheme = arch._theme !== expectedTheme;
        const hasStaleStackshiftFlag =
          selections.theme !== 'stackshift' && arch.isStackShift === true;
        const hasStaleDarkTokens =
          selections.theme !== 'stackshift' && tailwind && 'darkColorTokens' in tailwind;
        const hasStaleStandards = staleStandardKeys.length > 0;

        const themeChanged =
          hasStaleTheme || hasStaleStackshiftFlag || hasStaleDarkTokens || hasStaleStandards;
        if (themeChanged) {
          try {
            arch._theme = expectedTheme;
            if (hasStaleStackshiftFlag) arch.isStackShift = false;
            if (hasStaleDarkTokens && tailwind) delete tailwind.darkColorTokens;
            if (designStandards) {
              for (const k of staleStandardKeys) delete designStandards[k];
            }
            // Drop scan-synthesized patterns — they reference the prior
            // theme's primitives. Next /forge-scan repopulates for the
            // current theme.
            if ('patterns' in arch) delete arch.patterns;
    ...
    if (selections.wantsScan) {
      ...
      if (scanScript) {
        ...
        if (selections.wantsThemeOverride && selections.theme === 'stackshift') {
          ...
          if (selections.wantsNoBackup) scanArgs.push('--no-backup');
        }
        console.log('\nRunning quick scan…');
 ...
 */
function groupForFile(rel: string, features: FeatureId[]): string {
  if (rel.startsWith('themes/')) return 'theme';
  if (rel.startsWith('scripts/scan')) return 'scan';
  if (rel.startsWith('scripts/invoke') || rel.startsWith('scripts/apply-synthesis')) return 'forge';
  if (rel.startsWith('scripts/verify') || rel.startsWith('scripts/validate-contract') || rel.startsWith('packages/variant-contract')) return 'verify';
  if (rel.startsWith('scripts/export-design')) return 'export-design';
  if (rel.startsWith('scripts/fetch-handoff')) return 'fetch-handoff';
  if (rel.startsWith('scripts/mcp-server')) return 'mcp-server';
  return 'always';
}
