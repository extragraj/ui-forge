 ...
 */
const PRUNABLE_GROUPS: FeatureId[] = [
  'export-design',
  'fetch-handoff',
...
export function prunePriorFeatures(args: PruneArgs): PruneResult {
  const { cwd, priorFiles, nextFeatures, dryRun } = args;
  const removed: PrunedFile[] = [];
  const skipped: { path: string; reason: string }[] = [];

  const nextSet = new Set<FeatureId>(nextFeatures);
  // Required features cannot be pruned even if somehow absent from `nextFeatures`.
  for (const r of REQUIRED_FEATURES) nextSet.add(r);
  ...
  for (const [group, entries] of Object.entries(priorFiles)) {
    if (!PRUNABLE_GROUPS.includes(group as FeatureId)) {
      for (const e of entries) retainedFiles.add(e);
    } else if (nextSet.has(group as FeatureId)) {
      for (const e of entries) retainedFiles.add(e);
    }
  }
  ...
  for (const group of PRUNABLE_GROUPS) {
    ...
    for (const rel of entries) {
      ...
      try {
        ...
        skipped.push({ path: rel, reason: `error: ${(err as Error).message}` });
      }
    }
  }
  ...
  for (const feature of removedFeatures) {
    ...
    for (const platformId of platformIds) {
      ...
      for (const scopeChoice of expandScope(scope)) {
        ...
        if (!isOwnedCommand(absPath)) continue;
...
export interface PrunePlatformsArgs {
  cwd: string;
  homedir: string;
  priorScope: 'project' | 'global';
  priorPlatforms: string[];
 ...
 */
export function prunePlatforms(args: PrunePlatformsArgs): PruneResult {
  const { cwd, homedir, priorScope, priorPlatforms, nextPlatforms, priorFiles, dryRun } = args;
  const removed: PrunedFile[] = [];
  const skipped: { path: string; reason: string }[] = [];

  const nextSet = new Set<string>(nextPlatforms);
  ...
  for (const removedId of removedIds) {
    ...
    for (const lockedPath of allLockPaths) {
      ...
      if (!posixLocked.startsWith(skillPrefix + '/') && posixLocked !== skillPrefix) continue;
      const abs =
        priorScope === 'global'
      ...
      try {
        ...
        skipped.push({ path: lockedPath, reason: `error: ${(err as Error).message}` });
      }
    }
    if (!dryRun) {
    ...
    if (existsSync(paths.commandsDir)) {
      ...
      if (!dryRun) pruneEmptyDirs(paths.commandsDir);
    }
...
export function pruneOrphanedCommands(args: {
  ...
  const ownerByFile = new Map<string, FeatureId>();
  for (const [feature, fileName] of Object.entries(FEATURE_COMMANDS)) {
    if (fileName) ownerByFile.set(fileName, feature as FeatureId);
  }
  const nextSet = new Set<FeatureId>(nextFeatures);
  ...
  for (const platformId of platformIds) {
    ...
    for (const scopeChoice of expandScope(scope)) {
      const paths = platformPaths(cwd, platform, scopeChoice, homedir);
      if (!existsSync(paths.commandsDir)) continue;
      let files: string[] = [];
      try {
        files = readdirSync(paths.commandsDir).filter(
          (f) => f.startsWith('forge') && f.endsWith('.md')
        );
      } catch { continue; }
      for (const file of files) {
        const owner = ownerByFile.get(file);
        if (!owner) continue; // unknown command file → leave alone
        if (nextSet.has(owner)) continue; // feature still selected → keep
        const abs = join(paths.commandsDir, file);
        if (!isOwnedCommand(abs)) continue; // user-customized → preserve
...
export function pruneOrphanedSkillDirs(args: {
  ...
  for (const platform of PLATFORMS) {
    if (nextSet.has(platform.id)) continue;
    ...
    const hasSentinel =
      existsSync(join(paths.skillDir, 'scripts', 'scan.js')) ||
    ...
    if (dryRun) {
      ...
      try {
        ...
        if (existsSync(parent) && isDirEmpty(parent)) removeDir(parent);
      } catch (err) {
        skipped.push({ path: rel, reason: `error: ${(err as Error).message}` });
      }
    }
