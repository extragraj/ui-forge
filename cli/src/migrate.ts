...
import { loadLockfile, lockfileExists } from './lockfile.js';
import { detectPlatforms, platformById, platformPaths } from './platforms.js';
import { runInit } from './install.js';

export async function runMigrate(cwd: string, flags: ParsedFlags): Promise<void> {
  ...
  const home = homedir();
  const featureMap: Record<string, FeatureId> = {
    'forge.md': 'forge',
    'forge-scan.md': 'scan',
  ...
  const featuresSet = new Set<FeatureId>(['scan', 'forge']);
